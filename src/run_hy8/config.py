...
def project_from_mapping(config: Mapping[str, Any]) -> Hy8Project:
    ...
    project.units = _parse_unit_system(
        project_section.get("units", UnitSystem.ENGLISH.cli_flag)
    )
    project.exit_loss_option = int(project_section.get("exit_loss_option", 0))

    crossings_data = config.get("crossings", [])
    if not isinstance(crossings_data, Sequence) or isinstance(
        crossings_data, (str, bytes)
    ):
...
def _parse_crossing(entry: Mapping[str, Any]) -> CulvertCrossing:
    ...
    if not isinstance(culvert_entries, Sequence) or isinstance(
        culvert_entries, (str, bytes)
    ):
...
def _parse_tailwater(entry: Mapping[str, Any]) -> TailwaterDefinition:
    ...
    tailwater.constant_elevation = float(
        entry.get("constant_elevation", tailwater.constant_elevation)
    )
    tailwater.invert_elevation = float(
        entry.get("invert_elevation", tailwater.invert_elevation)
    )
    return tailwater
...
def _parse_roadway(entry: Mapping[str, Any]) -> RoadwayProfile:
    ...
    if "surface" not in entry:
        raise ValueError(
            "Roadway surface must be specified (paved, gravel, user_defined)."
        )
...
def _parse_culvert(entry: Mapping[str, Any], *, crossing_name: str) -> CulvertBarrel:
    ...
    culvert.material = _parse_culvert_material(
        entry.get("material", culvert.material.name)
    )
    culvert.number_of_barrels = int(
        entry.get("number_of_barrels", culvert.number_of_barrels)
    )
    culvert.inlet_invert_station = float(
        entry.get("inlet_invert_station", culvert.inlet_invert_station)
    )
    culvert.inlet_invert_elevation = float(
        entry.get("inlet_invert_elevation", culvert.inlet_invert_elevation)
    )
    culvert.outlet_invert_station = float(
        entry.get("outlet_invert_station", culvert.outlet_invert_station)
    )
    culvert.outlet_invert_elevation = float(
        entry.get("outlet_invert_elevation", culvert.outlet_invert_elevation)
    )
    culvert.roadway_station = float(
        entry.get("roadway_station", culvert.roadway_station)
    )
